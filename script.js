class AudioController {
    constructor() {
        this.bgMusicList = [
            new Audio('music/yougetlucky.mp3'),
            new Audio('music/babyrun.mp3'),
            new Audio('music/เฉี่ยว.mp3'),
            new Audio('music/kissถึง.mp3'),
            new Audio('music/แอบเผลอ.mp3'),
        ];
        this.bgMusicList.forEach(music => {
            music.volume = 0.5;
            music.loop = true;
        });
    ...
    startMusic() {
        this.stopMusic(); // หยุดเพลงก่อนหน้า

        // สุ่มเพลงใหม่
        const randomIndex = Math.floor(Math.random() * this.bgMusicList.length);
        this.bgMusic = this.bgMusicList[randomIndex];
        this.bgMusic.play();
